...
semitrailerTires = {p.tyre.semitraileraxle1.pos.L, p.tyre.semitraileraxle1.pos.R, 
                    p.tyre.semitraileraxle2.pos.L, p.tyre.semitraileraxle2.pos.R, 
                    p.tyre.semitraileraxle3.pos.L, p.tyre.semitraileraxle3.pos.R};
for i = 1:length(semitrailerTires)
    curvexy = [semitrailerTires{i}(:,1), semitrailerTires{i}(:,2)];
    [~, distances_center{i}] = distance2curve(curvexy, mapxy_center);
end
...
figure('Name', 'Distances to Roundabout Boundaries'); 
hold on;

% Plotting distances
for i = 1:length(tire_positions)
    % Plot distance to outer boundary
    subplot(2,1,1); 
    plot(distances_out{i}, 'LineWidth', 1);
    hold on;
    
    % Plot distance to inner boundary
    subplot(2,1,2);
    plot(distances_in{i}, 'LineWidth', 1);
    hold on;
end

% Setting up the plot : outer boundary distances
subplot(2,1,1);
title('Distances to Outer Boundary');
xlabel('Sample Point');
ylabel('Distance (m)');
xlim auto;
grid on;grid minor;

% Setting up the plot : inner boundary distances
subplot(2,1,2);
title('Distances to Inner Boundary');
xlabel('Sample Point');
ylabel('Distance (m)');
xlim auto;grid on;grid minor;
...
overall_roundabout_flag = checkFlags(distances_out, distances_in); % Determining the overall flag
