load ../simresults/truck_dbtrailer_SingleSine_v88_f40.mat
...
subplot(2,2,3)
plot(s.time,s.steeraxle.acc(:,2),'b-',s.time,s.trailer1axle2.acc(:,2),'k--','LineWidth',1.5);
grid on;
legend('Steer axle','semi-trailer 2 axle3',1);
title('Lateral accelerations of the first and last axle');
xlabel('t [s]');ylabel('ay [m/s2]');

RA=max(abs(s.trailer1axle2.acc(:,2)))/max(abs(s.steeraxle.acc(:,2)))
...
print -depsc -tiff ss_truck_dbtrailer
